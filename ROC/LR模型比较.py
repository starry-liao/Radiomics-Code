...
import pingouin as pg   # 需要安装：pip install pingouin
# from pauc import roc_auc_ci_score     # 用于计算 AUC 及置信区间
...
def delong_auc_ci(y_true, y_pred):
    ...
    ci_lower = max(0, auc_val - z * se)  # 截断至[0,1]
...
plt.title('ROC Curves Comparison (LR)', fontsize=14, fontweight='bold', pad=12)
