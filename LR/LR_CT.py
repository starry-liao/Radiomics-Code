...
# TODO: 请替换为您的CSV文件路径
train_df = pd.read_csv(r"REPLACE_WITH_YOUR_FILE.csv")   # 包含所有CT特征和标签
# TODO: 请替换为您的CSV文件路径
test_df = pd.read_csv(r"REPLACE_WITH_YOUR_FILE.csv")     # 包含所有CT特征和标签
...
print(f"CT特征数：{len(existing_features)}")
...
param_grid = {
    'lr__C': np.logspace(-1, 2, 20),
    'lr__penalty': ['l2'],               # 逻辑回归常用L2正则化
    'lr__class_weight': ['balanced', None],
    'lr__solver': ['lbfgs', 'liblinear'],# lbfgs支持L2正则化，适合小数据集
}
...
# print("\n最佳模型 Pipeline 已保存为 best_lr_ct_pipeline.pkl")
