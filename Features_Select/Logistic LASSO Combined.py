...
lasso_cv = LogisticRegressionCV(
    penalty='l1',
    solver='saga',          # saga支持L1和大数据
    Cs=20,                  # 搜索20个不同的C值
    cv=5,                   # 5折交叉验证
    scoring='roc_auc',      # 以AUC为标准选择最佳C
    max_iter=5000,
    random_state=42,
    n_jobs=-1
)
...
# 保存最终特征列表
# with open('final_combined_features.txt', 'w') as f:
...
for i, feat in enumerate(X_uni.columns):
    ...
    plt.plot(np.log10(C_grid), coef_path[:, i], alpha=0.4, linewidth=0.8, label=label)
